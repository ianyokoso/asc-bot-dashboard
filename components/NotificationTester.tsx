        ...
        <div className="relative mt-8 group">
            ...
            <div className="relative z-10 p-8">
                <h3 className="text-lg font-bold mb-6 flex items-center gap-2 text-gray-800">
                    <div className="p-2 bg-indigo-500/10 rounded-lg backdrop-blur-md border border-indigo-500/20">
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-3">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider pl-1">Target Audience</label>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <label className={`cursor-pointer group relative overflow-hidden rounded-xl border transition-all duration-300 ${targetType === 'me' ? 'bg-indigo-50/50 border-indigo-500/50 shadow-sm ring-1 ring-indigo-500/20' : 'bg-white/40 border-white/60 hover:bg-white/60'}`}>
                                <div className="p-4 flex items-center gap-3">
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${targetType === 'me' ? 'border-indigo-500 bg-indigo-500' : 'border-gray-300 bg-transparent'}`}>
                                        {targetType === 'me' && <div className="w-2 h-2 bg-white rounded-full"></div>}
                                    </div>
                                    <input type="radio" checked={targetType === 'me'} onChange={() => setTargetType('me')} className="hidden" />
                                    <span className={`font-medium ${targetType === 'me' ? 'text-indigo-900' : 'text-gray-600'}`}>내 DM으로만 보내기</span>
                                </div>
                            </label>
                            ...
                            <label className={`cursor-pointer group relative overflow-hidden rounded-xl border transition-all duration-300 ${targetType === 'user' ? 'bg-indigo-50/50 border-indigo-500/50 shadow-sm ring-1 ring-indigo-500/20' : 'bg-white/40 border-white/60 hover:bg-white/60'}`}>
                                <div className="p-4 flex items-center gap-3">
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${targetType === 'user' ? 'border-indigo-500 bg-indigo-500' : 'border-gray-300 bg-transparent'}`}>
                                        {targetType === 'user' && <div className="w-2 h-2 bg-white rounded-full"></div>}
                                    </div>
                                    <input type="radio" checked={targetType === 'user'} onChange={() => setTargetType('user')} className="hidden" />
                                    <span className={`font-medium ${targetType === 'user' ? 'text-indigo-900' : 'text-gray-600'}`}>특정 사용자에게 보내기</span>
                                </div>
                            </label>
                        </div>
                        ...
                        <div className={`transition-all duration-500 ease-in-out relative z-50 ${targetType === 'user' ? 'max-h-20 opacity-100 mt-2 transform translate-y-0' : 'max-h-0 opacity-0 -translate-y-2 pointer-events-none'}`}>
                            <div className="relative">
                                <button
                                    ...
                                    className="w-full text-left p-3 pl-4 border border-indigo-100/50 rounded-xl bg-white/60 backdrop-blur-sm hover:bg-white/80 focus:outline-none flex justify-between items-center transition-all shadow-sm ring-1 ring-transparent focus:ring-indigo-500/30 group"
                                >
                                    <span className={selectedUserId ? "text-indigo-900 font-medium" : "text-gray-400"}>
                                        {selectedUserId
                                            ? (() => {
                                    ...
                                    <span className={`text-gray-400 group-hover:text-indigo-500 transition-transform duration-300 ${isDropdownOpen ? 'rotate-180' : ''}`}>▼</span>
                                </button>

                                {/* Dropdown Panel (Absolute, outside overflow hidden) */}
                                {isDropdownOpen && targetType === 'user' && (
                                    <div className="absolute z-[60] w-full mt-2 bg-white/95 backdrop-blur-2xl border border-indigo-100 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] ring-1 ring-black/5 flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top">
                                        <div className="p-3 border-b border-indigo-50 bg-indigo-50/50">
                                            <input
                                                type="text"
                                                placeholder="이름, 닉네임, ID 검색..."
                                                className="w-full p-2 text-sm bg-white border border-indigo-100 rounded-lg focus:ring-2 focus:ring-indigo-500/20 outline-none text-gray-700 placeholder-gray-400"
                                                value={searchTerm}
                                            ...
                                            {Array.isArray(members) && members
                                                ...
                                                .map(m => (
                                                    <div
                                                        ...
                                                        className={`p-2.5 rounded-lg text-sm cursor-pointer transition-all flex justify-between items-center ${selectedUserId === m.discordId ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50'}`}
                                                    >
                                                        <div className="flex flex-col">
                                                            <span className="font-semibold text-gray-900">{m.discordNickname || m.name}</span>
                                                            <span className="text-xs text-gray-400">@{m.discordUsername || 'unknown'}</span>
                                                        </div>
                                                        <span className={`text-xs px-2 py-0.5 rounded-full ${selectedUserId === m.discordId ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-500'}`}>{m.track || 'No Track'}</span>
                                                    </div>
                                                ))}
                    ...
                    <div className="space-y-3">
                        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider pl-1">Notification Type</label>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {['숏폼 미제출 리마인더', '주간 과제 1차 리마인더 (오후)', '주간 과제 2차 리마인더 (저녁)', '패널티 경고 (4회)', '패널티 탈락 (5회)'].map((type) => (
                                <label key={type} className={`cursor-pointer group relative overflow-hidden rounded-xl border transition-all duration-200 ${msgType === type ? 'bg-indigo-50/50 border-indigo-500/50 shadow-sm ring-1 ring-indigo-500/20' : 'bg-white/40 border-white/60 hover:bg-white/60 text-gray-600'}`}>
                                    <div className="p-3 flex items-center justify-center text-center h-full gap-2">
                                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${msgType === type ? 'border-indigo-500 bg-indigo-500' : 'border-gray-300 bg-transparent'}`}>
                                            {msgType === type && <div className="w-1.5 h-1.5 bg-white rounded-full"></div>}
                                        </div>
                                        <input type="radio" name="msgType" checked={msgType === type} onChange={() => setMsgType(type)} className="hidden" />
                                        <span className={`text-sm font-medium ${msgType === type ? 'text-indigo-900' : ''}`}>{type}</span>
                                    </div>
                                </label>
                            ))}
                        </div>
                    ...
                    <button
                        onClick={handleTest}
                        className="w-full py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-base shadow-[0_4px_14px_0_rgba(79,70,229,0.3)] hover:shadow-[0_6px_20px_rgba(79,70,229,0.23)] hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 flex items-center justify-center gap-2 group"
                    >
