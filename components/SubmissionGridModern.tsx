...
const SubmissionGridModern: React.FC<SubmissionGridProps> = ({ members, submissions, cohortConfig }) => {
    ...
    return (
        ...
        <div className="flex flex-col h-[calc(100vh-320px)] rounded-2xl bg-white/40 backdrop-blur-xl border border-white/50 shadow-2xl overflow-hidden relative">
            ...
            {/* 트랙 탭 (Glass Style) */}
            <div className="flex items-center px-6 pt-6 gap-2 overflow-x-auto pb-2 custom-scrollbar z-10">
                {trackOrder.map((track) => {
                    ...
                    return (
                        <button
                            ...
                            className={`
                px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap flex items-center gap-2 border
                ${isActive
                                    ? 'bg-white/80 border-indigo-200 text-indigo-700 shadow-lg scale-105 backdrop-blur-md'
                                    : 'bg-white/20 border-white/30 text-gray-500 hover:bg-white/40 hover:text-gray-700'
                                }
              `}
                        >
                            {track}
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${isActive ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-200/50 text-gray-500'
                                }`}>
                                {count}
                            </span>
                        </button>
                        ...
                        <table className="min-w-full divide-y divide-gray-200/50">
                            <thead className="bg-gray-50/80 backdrop-blur-md sticky top-0 z-30">
                                <tr>
                                    ...
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider sticky left-0 z-40 bg-gray-50/95 backdrop-blur-md shadow-[2px_0_5px_rgba(0,0,0,0.02)] border-b border-gray-200/50 min-w-[200px]">
                                        멤버
                                    </th>
                                    {displayDates.map((date) => (
                                        <th key={date} className="px-2 py-3 text-center min-w-[70px] border-b border-gray-200/50">
                                            {getDayLabel(date)}
                                        </th>
                                    ))}
                                </tr>
                                ...
                                {filteredMembers.length > 0 ? (
                                    filteredMembers.map((member, idx) => (
                                        <tr key={member.id} className={`transition-colors hover:bg-indigo-50/30 ${idx % 2 === 0 ? 'bg-white/20' : 'bg-transparent'}`}>
                                            ...
                                            <td className="px-4 py-3 sticky left-0 z-20 bg-white/90 backdrop-blur-md shadow-[2px_0_5px_rgba(0,0,0,0.02)] border-r border-gray-100/50">
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-bold text-gray-800">{member.name}</span>
                                                    <span className="text-[10px] text-gray-400 font-mono mt-0.5">{member.discordUsername || member.discordId}</span>
                                                </div>
                                            </td>
                                            ...
                                            {displayDates.map((date) => {
                                                ...
                                                return (
                                                    <td key={`${member.id}-${date}`} className={`p-2 border-r border-gray-100/30 last:border-r-0 text-center relative ${isHoliday ? 'bg-rose-50/20' : ''}`}>
                                                        {isHoliday ? (
                                                            <span className="text-gray-300">-</span>
                                                        ) : (
                                                            getStatusChip(displayStatus, sub?.link, date)
                                                        )}
                                                    </td>
                                                );
                                            })}
